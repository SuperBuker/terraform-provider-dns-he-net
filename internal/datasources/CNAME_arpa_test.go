...
func TestAccArpaCNAME(t *testing.T) {
	record, ok := ArpaZoneRecords["CNAME"]
	if !ok {
		...
		Steps: []resource.TestStep{
			// Read testing
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr("data.dns-he-net_cname.record-cname", "zone_id", fmt.Sprint(ArpaZone.ID)),
				),
			},
		},
	})
