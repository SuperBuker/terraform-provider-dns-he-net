		...
		Steps: []resource.TestStep{
			...
			// Validates data default value by setting dynamic to true
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr("dns-he-net_a.record-a", "zone_id", fmt.Sprint(DomainZone.ID)),
			...
			// Validates ddns update by retrieving the new data value
			{
				PreConfig: func() {
					...
					ok, err := cli.DDNS().UpdateIP(context.Background(), domainUpdate, password, "10.2.3.4")
					require.NoError(t, err)
					assert.True(t, ok)
				},
				Config: ProviderConfig +
