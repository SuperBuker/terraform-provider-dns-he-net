...
func TestAccArpaDDNSKey(t *testing.T) {
	t.Parallel()
		...
		Steps: []resource.TestStep{
			...
			// Validate defaults
			{
				Config: ProviderConfig +
					...
					domain = %q
					key = %q
				}`, ArpaZone.ID, domainInit, password),
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr("dns-he-net_ddnskey.ddnskey", "zone_id", fmt.Sprint(ArpaZone.ID)),
			...
			// Update and Read testing
			{
				Config: ProviderConfig +
					...
					domain = %q
					key = %q
				}`, ArpaZone.ID, domainUpdate, password),
				Check: resource.ComposeAggregateTestCheckFunc(
					// Verify record attributes
